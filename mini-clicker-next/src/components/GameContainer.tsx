...
const WelcomeText = styled.h1`font-size: 32px`;
const WelcomeSubtitle = styled.p`font-size: 18px`;
...
export default function GameContainer() {
    ...
    const loadUsersData = useCallback(async (): Promise<UsersData> => {
        try {
            ...
            if (response.ok) {
                ...
                return {
                    users: Object.fromEntries(
                        result.data.topPlayer.map((user: any) => [user.userId.toString(), user])
                    ),
            ...
            try {
                ...
                return saved ? JSON.parse(saved) : { users: {}, lastUpdated: new Date().toISOString() }
            } catch {
    ...
    const saveUsersData = useCallback(async (data: UsersData) => {
        try {
            const response = await fetch('/api/users/stats', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                throw new Error('Falied to save users data')
            }
        } catch (error) {
    ...
    const updateUserStats = useCallback((user: TelegramUser, gameStats: GameState) => {
        ...
        const updatedStats: UserStats = {
            ...
            clickPowerLevel: Math.max(existingStats?.clickPowerLevel || 0, gameStats.clickPowerLevel),
            autoClickerLevel: Math.max(existingStats?.autoClickerLevel || 0, gameStats.autoClickerLevel),
    ...
    const exportUserData = async () => {
        try {
            // 1. Запрос к серверу для экспорта
            ...
            console.log('Ошибка экспорта данных:', error);
            const dataToExport = {
                gameState,
                userData: usersData,
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                console.log('Ошибка загрузки пользовательских данных', error)
            }
        }
    ...
    useEffect(() => {
        const initializeTelegram = async () => {
            if (typeof window !== 'undefined' && (window as any).Telegram) {
                ...
                const userStats = getUserStats(telegramUser.id)
                if (userStats) {
    ...
    useEffect(() => {
        // 1. Ждем готовности Telegram
        if (!isTelegramReady) return
        // 2. Загружаем сохраненное состояние
        const savedState = localStorage.getItem('miniClickerState')
        if (savedState) {
            const parsedState = JSON.parse(savedState)
            // 3. Синхронизация autoClicker
            if (parsedState.autoClickerLevel && parsedState.autoClickerLevel !== parsedState.autoClicker) {
                parsedState.autoClicker = parsedState.autoClickerLevel
            }
            // 4. Восстановление состояния игры
    ...
    const getClickPowerCost = () => {
        return Math.floor(10 * Math.pow(1.5, gameState.clickPowerLevel))
    }

    const getAutoClickerCost = () => {
        return Math.floor(50 * Math.pow(2, gameState.autoClickerLevel))
    }
    ...
    const handleClick = () => {
        ...
        setTimeout(() => setIsButtonAnimated(false), 100)
    }
    ...
    const upgradeClickPower = () => {
        ...
        if(gameState.balance >= cost){
            setGameState(prevState => ({
                ...
                clickPowerLevel: prevState.clickPower + 1,
    ...
    const buyAutoClicker = () => {
        ...
        if(gameState.balance >= cost){
            setGameState(prevState => ({
                ...
                autoClicker: prevState.autoClickerLevel
            }))
        }
    }
    ...
    if(showWelcome && user){
        return(
            <WelcomeOverlay>
                ...
                <WelcomeSubtitle>Добро пожаловать в Mini Clicker!<br /> Нажми на кнопку чтобы начать</WelcomeSubtitle>
        ...
        <Container className='telegram-web-app scroll-container'>
                    ...
                    {(() => {
                        ...
                        return stats ? (
                            <div style ={{marginTop: "10px", fontSize: "12px", color: "#666"}}>
                                ...
                                <div>Всего клипов: {stats.totalClicks.toLocaleString()}</div>
                            </div>
                        ) : null
                    })()}
                </UserInfo>
            ...
            <MainButtonContainer>
                <ClickButton
                    onClick={handleClick}
                    style={{
                        animation: isButtonAnimated ? 'none' : `none`,
                        transform: isButtonAnimated ? 'scale(0.95)' : 'scale(1)',
                    }}
                >
